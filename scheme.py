...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if not isinstance(first, Pair):
            if isinstance(env.lookup(first), MacroProcedure):
                return scheme_eval(env.lookup(first).apply_macro(rest, env), env, True)
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        eval_1st = scheme_eval(first, env)
        check_procedure(eval_1st)
        args = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(eval_1st, args, env)
        # END PROBLEM 5
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        "*** YOUR CODE HERE ***"
        value_eval = scheme_eval(expressions.second.first, env)
        env.define(target, value_eval)
        return target
        # END PROBLEM 6
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        value = LambdaProcedure(target.second, expressions.second, env)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        symbols, values = Pair(clause.first, symbols), Pair(scheme_eval(clause.second.first, env), values)
...
def do_define_macro(expressions, env):
    ...
    if isinstance(expressions.first, Pair):
        if not isinstance(expressions.first.first, int):
            if expressions.second:
                env.define(expressions.first.first, MacroProcedure(expressions.first.second, expressions.second, env))
